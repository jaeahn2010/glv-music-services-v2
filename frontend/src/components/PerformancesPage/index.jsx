        ...
        <main className={`${isMenuOpen ? 'z-0 opacity-5' : ''} font-poppins min-h-[125vh]`}>
            ...
            ? <section className="career-items my-24 min-h-[25rem]">
                ...
                {performances.map((performance, index) => 
                    <div key={index} className='card-flip flex my-5 w-11/12 md:w-3/4 h-[5rem] md:h-[10rem] mx-auto border-2 border-stone-200 bg-gradient-to-r from-stone-800 via-stone-500 to-stone-800'>
                        <div className="card-front flex w-full">
                            <div className="w-full flex flex-col items-center justify-center">
                                ...
                                <div className='flex justify-between items-center w-5/6'>
                                    ...
                                    <p className={performance.date > new Date() ? 'text-green-300' : 'text-red-403'}>{new Date(performance.date).toLocaleDateString()}, {nonMilitaryTime(performance.time)}</p>
                                </div>
                            ...
                            <div>  
                                ...
                                ? <>
                                    ...
                                    {performance.collaborators.map(collaborator =>
                                        <p key={collaborator.collaboratorLastName + collaborator.collaboratorFirstName}>{collaborator.collaboratorFirstName} {collaborator.collaboratorLastName} ({collaborator.collaboratorInstrument})</p>
                                    )}
                                </>
                                : ''}
                            </div>	
